...
def train_and_evaluate_with_module(hub_module, train_input_fn, predict_train_input_fn, train_module=False):
        ...
        return {"training accuracy": training_set_accuracy}
...
def main():
    ...
    with tf.Graph().as_default():
        cm = tf.confusion_matrix(train_df["emoji"], 
    ...
    results["nnlm-en-dim128"] = train_and_evaluate_with_module(
        "https://tfhub.dev/google/nnlm-en-dim128/1", train_input_fn, predict_train_input_fn)
    results["nnlm-en-dim128-with-module-training"] = train_and_evaluate_with_module(
        "https://tfhub.dev/google/nnlm-en-dim128/1", train_input_fn, predict_train_input_fn, True)
    results["random-nnlm-en-dim128"] = train_and_evaluate_with_module(
        "https://tfhub.dev/google/random-nnlm-en-dim128/1", train_input_fn, predict_train_input_fn)
    results["random-nnlm-en-dim128-with-module-training"] = train_and_evaluate_with_module(
        "https://tfhub.dev/google/random-nnlm-en-dim128/1", train_input_fn, predict_train_input_fn, True)
    print(results)
